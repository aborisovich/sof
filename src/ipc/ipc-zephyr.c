...
static int ipc_device_suspend_handler(const struct device *dev, void *arg)
{
	...
	// we are not entering D3 - power state change is not caused by IPC message
	// from host that would disable the lock.
...
#if defined(CONFIG_PM)
		// unlock policy locks set during firmware init
